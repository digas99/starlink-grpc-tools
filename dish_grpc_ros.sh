...
if [ -z "$(docker images -q starlink/dish_grpc_ros)" ] || [ "$BUILD" = true ]; then
	...
	docker build -t starlink/dish_grpc_ros -f ros/dish_grpc_ros.dockerfile .
fi

# if not BUILD
if [ -z "$BUILD" ] || [ "$RUN" = true ]; then
	echo "Running starlink/dish_grpc_ros"
	docker run -it --rm --network host --privileged --name dish_grpc_ros starlink/dish_grpc_ros
fi